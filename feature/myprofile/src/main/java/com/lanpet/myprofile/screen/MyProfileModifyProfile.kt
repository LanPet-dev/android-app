...
private fun NickNameSection() {
    ...
    Spacer(modifier = Modifier.padding(top = LanPetDimensions.Margin.xSmall))
    nicknameDuplicateCheckStatus?.let {
        if (it) {
            DuplicatedNicknameOkText(
                stringResource(R.string.nickname_check_ok_string)
            )
        } else {
            DuplicatedNicknameErrorText(
                stringResource(R.string.nickname_check_error_string)
            )
        }
    }
...
@Composable
@PreviewLightDark
fun PreviewMyProfileModifyProfile() {
    LanPetAppTheme {
